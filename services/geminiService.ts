 ...
 */
export const generatePracticePhrases = async (language: Language, focus: string): Promise<string[]> => {
  ...
  if (localPhrases && localPhrases.length > 0) {
    // Barajamos y devolvemos 5 frases de la lista local
    return [...localPhrases].sort(() => Math.random() - 0.5).slice(0, 5);
  }
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `Genera 5 frases cortas y útiles para practicar mecanografía en idioma ${language}. 
      El enfoque es: ${focus}. 
